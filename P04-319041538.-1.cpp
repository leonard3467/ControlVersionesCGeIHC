...
int main()
{
	...
	*/
	...
	glm::mat4 modelaux2(1.0);// matriz 2 de aooyo para no tener problemas con los demas 
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//------------------------articulación 1-------------------------
		model = glm::translate(model, glm::vec3(0.4f, 6.0f, -4.0f));
		//rotación alrededor de la articulación que une con la cabina
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion1()), glm::vec3(0.0f, 0.0f, 1.0f));
		// modeles una matriz que puede rotarse con la articulacion 
		modelaux = model;
		...
		// model es una matriz trsladada y rotada 
		modelaux = model; //La matriz auxiliar guarda todas las transformaciones anteriores
		// model aux tambien es una matriz rotada
		model = glm::scale(model, glm::vec3(5.0f, 1.0f, 1.0f)); //Se estableció 5 
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));		
		...
		//-------------------------articulación 2----------------
		model = glm::translate(model, glm::vec3(2.5f, 0.0f, 0.0f));//Poniendo artoculación en la punta del brazo
		...
		//-----------------------------segundo brazo-------------------------------------
		model = modelaux; //para descartar la escala que no quiero heredar se carga la información de la matriz auxiliar
		...
		//-----------------------------canasta-------------------------------------
		model = modelaux; //para descartar la escala que no quiero heredar se carga la información de la matriz auxiliar
		model = glm::translate(model, glm::vec3(0.0f, -0.5f, 0.0f)); //se posiciona la canasta
		model = glm::scale(model, glm::vec3(2.5f, 2.5f, 2.5f)); //escalamos para cubrir todas las paredes de la canasta
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));

		// Cambia el color a morado
		color = glm::vec3(0.5f, 0.0f, 0.5f); // Morado (mezcla de rojo y azul)
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); // Cambia el color de los objetos

		meshList[0]->RenderMesh(); // Renderiza un solo cubo escalado que representa toda la canasta
